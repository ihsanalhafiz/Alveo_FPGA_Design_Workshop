...
// ==============================================================
`timescale 1ns/1ps
module streamhls_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER                = 6'h08,
    ADDR_ISR                = 6'h0c,
    ADDR_HBM_DATA_DATA_0    = 6'h10,
    ADDR_HBM_DATA_DATA_1    = 6'h14,
    ADDR_HBM_DATA_CTRL      = 6'h18,
    ADDR_OUTPUT_DATA_DATA_0 = 6'h1c,
    ADDR_OUTPUT_DATA_DATA_1 = 6'h20,
    ADDR_OUTPUT_DATA_CTRL   = 6'h24,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_HBM_DATA_DATA_0: begin
                    rdata <= int_hbm_data[31:0];
                end
                ADDR_HBM_DATA_DATA_1: begin
                    rdata <= int_hbm_data[63:32];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_HBM_DATA_DATA_0)
            int_hbm_data[31:0] <= (WDATA[31:0] & wmask) | (int_hbm_data[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_hbm_data[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_HBM_DATA_DATA_1)
            int_hbm_data[63:32] <= (WDATA[31:0] & wmask) | (int_hbm_data[63:32] & ~wmask);
    end
end
