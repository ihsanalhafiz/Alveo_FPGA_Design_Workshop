...
entity streamhls_control_s_axi is
generic (
...
end entity streamhls_control_s_axi;
...
architecture behave of streamhls_control_s_axi is
    ...
    constant ADDR_HBM_DATA_DATA_0    : INTEGER := 16#10#;
    constant ADDR_HBM_DATA_DATA_1    : INTEGER := 16#14#;
    constant ADDR_HBM_DATA_CTRL      : INTEGER := 16#18#;
    constant ADDR_OUTPUT_DATA_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_OUTPUT_DATA_DATA_1 : INTEGER := 16#20#;
    constant ADDR_OUTPUT_DATA_CTRL   : INTEGER := 16#24#;
    ...
    signal int_hbm_data        : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_HBM_DATA_DATA_0 =>
                        rdata_data <= RESIZE(int_hbm_data(31 downto 0), 32);
                    when ADDR_HBM_DATA_DATA_1 =>
                        rdata_data <= RESIZE(int_hbm_data(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    ...
    hbm_data             <= STD_LOGIC_VECTOR(int_hbm_data);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_HBM_DATA_DATA_0) then
                    int_hbm_data(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_hbm_data(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_HBM_DATA_DATA_1) then
                    int_hbm_data(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_hbm_data(63 downto 32));
                end if;
            end if;
        end if;
    end process;
